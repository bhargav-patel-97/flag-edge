...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!process.env.ALPACA_API_SECRET || !process.env.ALPACA_API_KEY) {
      throw new Error('Missing required Alpaca environment variables: ALPACA_API_SECRET and ALPACA_API_KEY');
    }
    ...
    const errorResponse = {
      ...
      environment: {
        ...
        has_alpaca_keys: !!(process.env.ALPACA_API_SECRET && process.env.ALPACA_API_KEY)
      }
    };
