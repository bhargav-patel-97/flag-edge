  ...
  async execute({ symbol = "QQQ", timeframe = "5Min", maxPositions = 3 }) {
    try {
      ...
      try {
        ...
        // Try to get data access debug info
      ...
      await this.logLevelsToDatabase(symbol, timeframe, levels, { ma200, ma400 });
  ...
  async logLevelsToDatabase(symbol, timeframe, levels, indicators) {
    try {
      const levelsData = {
        ...
        timeframe,
        timestamp: new Date().toISOString(),
        ma200: indicators.ma200 ? indicators.ma200[indicators.ma200.length - 1] : null,
        ma400: indicators.ma400 ? indicators.ma400[indicators.ma400.length - 1] : null,
      ...
      await this.db.logEvent('levels_detected', levelsData);
      console.log(`Logged ${levels.length} levels to database for ${symbol} ${timeframe}`);
    } catch (error) {
      console.error('Error logging levels to database:', error);
